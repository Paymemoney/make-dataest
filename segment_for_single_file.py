...
def find_segments(wav, sample_rate, min_duration, max_duration, max_gap_duration, threshold_db):
  ...
  s = segments
  while s is not None:
    # Exclude segments from the first 5 seconds -- these are the LibriVox disclaimer.
    # Also exclude segments that are too short or too long:
    if (s.end / sample_rate >= 5.0 and
...
#   with open(os.path.join('E:\CNTV\wave', source_name + '.txt'), encoding = 'utf-8') as f:
#     for line in f:
...
def build_segments(args):
  ...
  #   #   print('Loading %s: %s (%d of %d)' % (file_id, filename, i+1, len(filenames)))
    wav, sample_rate = librosa.load('C:\\Users\\pulpfiction\\Desktop\\3.acc')
    ...
    for j, s in enumerate(segments):
      ...
      s.set_filename_and_id('2019.6.25', '%s_%04d' % (1, j+1))
    ...
    for s in segments:
      ...
      sf.write(out_path, segment_wav, sample_rate)
...
def main():
  ...
  parser.add_argument('--base_dir', default='C:\\Users\\pulpfiction\\Desktop\\3.acc')
  parser.add_argument('--source', default='20190625759', help='Name of the source to process')
