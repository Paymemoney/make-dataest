...
def build_from_path(in_dir, out_dir, num_workers=1, tqdm=lambda x: x):
  '''Preprocesses the FemaleMandarin-1.0 dataset from a given input path into a given output directory.
  ...
  trn_files = []
  trn_files += glob.glob(os.path.join(in_dir, 'group1', '*.trn'))
  trn_files += glob.glob(os.path.join(in_dir, 'group2', '*.trn'))
  trn_files += glob.glob(os.path.join(in_dir, 'group3', '*.trn'))
  trn_files += glob.glob(os.path.join(in_dir, 'group4', '*.trn'))
