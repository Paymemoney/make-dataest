...
from datasets import blizzard, ljspeech, thchs30, femalemandarin
...
def preprocess_femalemandarin(args):
  in_dir = os.path.join(args.base_dir, 'FemaleMandarin-1.0')
  out_dir = os.path.join(args.base_dir, args.output)
  os.makedirs(out_dir, exist_ok = True)
  metadata = femalemandarin.build_from_path(in_dir, out_dir, args.num_workers, tqdm = tqdm)
  write_metadata(metadata,out_dir)
...
def main():
  ...
  parser.add_argument('--base_dir', default=os.path.expanduser('~/tacotron_update'))
  parser.add_argument('--output', default='training')
  parser.add_argument('--dataset', required=True, choices=['blizzard', 'ljspeech', 'thchs30', 'femalemandarin'])
  ...
  elif args.dataset == 'femalemandarin':
    preprocess_femalemandarin(args)
